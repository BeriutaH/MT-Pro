  ...
  <el-form label-width="100px">
      ...
      <el-form :inline="true">
        <el-form-item v-for="(item, index) in spuProductInfo" :key="item.id" :label="item.attrName">
          <el-select class="input_an_select" v-model="item.attrIdAndAttrValueId">
            <el-option :value="`${item.id}:${attr.id}`" v-for="(attr, index) in item.attrValueList" :key="attr.id" :label="attr.valueName"></el-option>
          </el-select>
        </el-form-item>
      </el-form>
    </el-form-item>
      ...
      <el-form :inline="true">
        <el-form-item v-for="(item, index) in spuSaleInfo" :key="item.id" :label="item.saleAttrName">
          <el-select class="input_an_select" v-model="item.saleIdAndValueId">
            <el-option :value="`${item.id}:${saleValue.id}`" v-for="(saleValue, index) in item.spuSaleAttrValueList" :key="saleValue.id" :label="saleValue.saleAttrValueName"></el-option>
          </el-select>
        </el-form-item>
      </el-form>
    </el-form-item>
        ...
        <el-table-column label="操作">
          <template #="{row, $index}">
            <template v-if="row.imgUrl === skuParams.skuDefaultImg">
              <el-button size="small" style="background-color:#f4b8e4; color: white">默认图片</el-button>
            </template>
            <template v-else>
              <el-button style="background-color:#48a7a7; color: white" size="small" @click="handler(row)">设置默认</el-button>
            </template>
          </template>
        </el-table-column>
      </el-table>
    ...
    <el-form-item>
      <div class="button_container">
        ...
        <el-button class="custom_button" type="primary" size="default" @click="addSku">确认</el-button>
      </div>
    </el-form-item>
  </el-form>
</template>
